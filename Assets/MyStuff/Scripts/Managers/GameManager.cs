...
public class GameManager : MonoBehaviour
{
    ...
    internal Dictionary<Coordinates, GameObject> LoadedRegions = new Dictionary<Coordinates, GameObject>();
    ...
    internal void LoadRegion(Coordinates coords)
    {
        var gameObject = Instantiate(Resources.Load($"Regions/Region{coords.X},{coords.Y}") as GameObject);
        LoadedRegions.Add(coords, gameObject);
    }

    internal void DestroyFarRegions(List<Coordinates> coordsToKeep)
    {
        List<Coordinates> coordsToRemove = new List<Coordinates>();
        foreach (var region in LoadedRegions)
        {
            if (!coordsToKeep.Contains(region.Key))
            {
                coordsToRemove.Add(region.Key);
            }
        }
        for (int i = 0; i < coordsToRemove.Count; i++)
        {
            if (LoadedRegions.TryGetValue(coordsToRemove[i], out var region))
            {
                Destroy(region);
                LoadedRegions.Remove(coordsToRemove[i]);
            }
        }
    }
}
